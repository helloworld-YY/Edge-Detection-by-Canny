...
class Canny:
        ...
        for i in range(self.x):
            for j in range(self.y):
                if j == 0:
                    ...
                    new_img_y[j][i] = np.sum(np.array([[self.img[j - 1][i]], [self.img[j][i]]]) * self.y_kneral) #数组相乘，保持维度一致，均为二维数组
                if i == 0:
                    new_img_x[j][i] = 1
                else:
                    new_img_x[j][i] = np.sum(np.array([[self.img[j][i - 1], self.img[j][i]]]) * self.x_kneral) #数组相乘，保持维度一致，均为二维数组
    ...
    def Hysteresis_thresholding(self):
        ...
        #用强边延伸弱边，延伸方向为梯度的垂直方向，注意延伸方向为垂直方向，故与上述非极大抑制，插值代码存在不同
    ...
    def canny_EdgeDetection(self):
        
        self.img = cv2.GaussianBlur(self.img, (self.Guassian_kernal_size, self.Guassian_kernal_size), 0)  #给定标准差为0时，其内部会调用getGaussianKernel()求得对应的sigma
